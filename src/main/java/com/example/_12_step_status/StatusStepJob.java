package com.example._12_step_status;
...
public class StatusStepJob {
    ...
    public Job job(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new JobBuilder("status-step-job", jobRepository)
                .start(firstStep(jobRepository, transactionManager))
                // .on("FAILED").to(failStep(jobRepository, transactionManager))
                .on("FAILED").end() // 正常結束 -- COMPLETED
                // .on("FAILED").fail() // 異常結束 -- FAILED
                // .on("FAILED").stopAndRestart(successStep(jobRepository, transactionManager)) // 中斷結束 -- STOPPED
